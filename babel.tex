...
\begin{frame}[t]{Business Model: Ecosystem as a Service}
    ...
    \begin{center}
    \begin{tikzpicture}[>=stealth, node distance=1.73*2.2 and2.2,   % Sqrt[3]≈1.73
    block/.style={circle, draw, text width=50, align=center, minimum height=50, fill=blue!20, font=\footnotesize}]
    
    % Nodes
    \node[block] (babel) {Blockchain };
    \node[block, on grid,  above left=of babel] (users) {Users};
    \node[block, on grid,  above right=of babel] (contributors) {Contributors\\(Miners)};
    
    % Paths
    \draw[->] (users) to[bend left=20] node[below, sloped] { tokens} (babel);
    \draw[->] (babel) to[bend left=20] node[below, sloped] { tokens} (contributors);
    \draw[->] (contributors) to[bend left=20] node[above, sloped] { tokens} (users);
    
    
    \draw[->] (contributors) to[bend left=30] node[below, sloped] {PoWoC improves AGI} (babel);
    \draw[->] (users) to[bend left=30] node[above, sloped] { money buys tokens} (contributors);
...
\newcounter{b}
\foreach \p/\t/\c in {
    10/Early Contributors/yellow!30,
    10/DAO Foundation/green!30,
    10/Community Incentive/orange!30,
        5/Founders/blue!30,
    15/VC/cyan!30,
    50/To Be Mined/red!30
                   }
  {
